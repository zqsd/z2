...
class Cell {
    ...
    static fromBoundsToGoogleTile(bounds) {
        for (let z = 26; z > 0; z--) {
            const x0 = long2tile(bounds.minLng, z), y0 = lat2tile(bounds.minLat, z), x1 = long2tile(bounds.maxLng, z), y1 = lat2tile(bounds.maxLat, z);
            if (x0 === x1 && y0 === y1)
    ...
    static fromLatLngToGoogleTile(lat, lng, z = 26) {
        ...
        const x = long2tile(lng, z), y = lat2tile(lat, z);
